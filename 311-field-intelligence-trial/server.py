...
class AppHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            self.json_response(
                HTTPStatus.OK,
                {"status": "ok", "storage": "sqlite", "authenticated": bool(self.current_session())},
            )
...
def create_server(host, port, database_path, static_root):
...
def main():
    ...
    server = create_server(
        args.host, args.port, args.database.resolve(), Path(__file__).resolve().parent
    )
    print(
        json.dumps(
            {
